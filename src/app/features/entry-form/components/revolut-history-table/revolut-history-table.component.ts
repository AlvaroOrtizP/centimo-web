...
@Component({
  selector: 'app-revolut-history-table',
  standalone: true,
  template: `
            ...
            @if (balances().length === 0) {
              ...
              @for (b of balances(); track b.id) {
                <tr class="transition-all duration-150 hover:bg-gray-50/80">
                  ...
                  <td class="px-4 py-3 text-right text-emerald-600">{{ b.dineroFinal ?? '-' }}</td>
...
export class RevolutHistoryTableComponent {
  readonly balances = input.required<RevolutBalance[]>();

  readonly headingTooltip = input<string | undefined>();

  readonly edit = output<RevolutBalance>();
