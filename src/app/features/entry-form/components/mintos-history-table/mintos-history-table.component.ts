...
@Component({
  selector: 'app-mintos-history-table',
  standalone: true,
  template: `
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-gray-100 bg-gray-50/80 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">
              ...
              <th class="px-4 py-3 text-right">Importe añadido</th>
          ...
          <tbody class="divide-y divide-gray-100">
            @if (balances().length === 0) {
              <tr>
                <td colspan="4" class="px-4 py-8 text-center text-sm text-gray-400">Sin registros todavía. Guarda un balance para este mes.</td>
              </tr>
            } @else {
              @for (b of balances(); track b.id) {
                <tr class="transition-all duration-150 hover:bg-gray-50/80">
                  ...
                  <td class="px-4 py-3 text-right font-semibold text-gray-900">{{ b.valorFinal.toLocaleString('es-ES') }} €</td>
                  <td class="px-4 py-3 text-right text-gray-500">{{ b.importeAnadido.toLocaleString('es-ES') }} €</td>
...
export class MintosHistoryTableComponent {
  readonly balances = input.required<MintosBalance[]>();

  readonly headingTooltip = input<string | undefined>();

  readonly edit = output<MintosBalance>();
