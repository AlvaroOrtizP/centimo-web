...
@Component({
  selector: 'app-b100-history-table',
  standalone: true,
  template: `
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-gray-100 bg-gray-50/80 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">
              ...
              <th class="px-4 py-3 text-right">Total a repartir</th>
            ...
            @if (balances().length === 0) {
              ...
              @for (b of balances(); track b.id) {
                <tr class="transition-all duration-150 hover:bg-gray-50/80">
                  ...
                  <td class="px-4 py-3 text-right font-medium text-emerald-600">{{ b.dineroTotalRepartir.toLocaleString('es-ES') }} €</td>
...
export class B100HistoryTableComponent {
  readonly balances = input.required<B100Balance[]>();

  readonly headingTooltip = input<string | undefined>();

  readonly edit = output<B100Balance>();
